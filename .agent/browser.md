...
- `src/app/inspectorOverview.ts` — 713 lines / 27.3 KiB — technologyReach:36, renderTechnologies:67, stockCompartments:73, …
...
- `src/core/FactoryHost.ts` — 398 lines / 11.7 KiB — FactoryWorkerMethod:28, WorldChoice:49, FactoryTransport:51, …
...
- `src/core/commands.ts` — 346 lines / 11.4 KiB — EncodedCommand:3, MAX_AIM_COORDINATE:13, halfTransfer:23, …
- `src/core/definitions.ts` — 914 lines / 32.1 KiB — supportsRecipe:14, reservationCells:47, footprintIsContiguous:65, …
- `src/core/directions.ts` — 32 lines / 1.4 KiB — TRANSPORT_DIRECTIONS:4, CORNER_START:7, DIRECTION_NAMES:10, …
- `src/core/factory.worker.ts` — 227 lines / 8.1 KiB — handle:54, requireFactory:190, delta:203, …
...
- `src/core/saveSlots.ts` — 812 lines / 26.3 KiB — SAVE_VERSION:53, SAVE_CATALOG_KEY:54, LEGACY_SAVE_PREFIX:55, …
...
- `src/core/types.ts` — 1398 lines / 47.2 KiB — BuildingKind:3, Terrain:15, Substrate:29, …
- `src/data/definitions.json` — 3144 lines / 73.7 KiB
- `src/data/scenarios.json` — 168 lines / 4.8 KiB
- `src/data/technologies.json` — 563 lines / 17.1 KiB
