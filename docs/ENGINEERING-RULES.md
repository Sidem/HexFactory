# HexFactory engineering rule ledger
...
This file preserves the detailed shipped invariants for targeted lookup. Do not preload it for every
task: start from `AGENTS.md` and `docs/AGENT-MAP.md`, then retrieve only the headings relevant to the
change. The next milestones live in `docs/HEXFACTORY-PLAN.md`, architecture decisions in
`docs/ARCHITECTURE.md`, art rules in `docs/ART.md`, and measured capacity in `docs/BENCHMARKS.md`.

## Where things stand

Shipped through **v0.39.0 Masonry Enclosures**. Current envelope versions,
all five of which native refuses a load on: **save 31, definitions 25, technologies 13, scenarios 7,
world generator 9**, and wire (snapshot delta) **17**. `SAVE_VERSION` is the one literal in the host,
