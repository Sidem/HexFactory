...
const RULES = [
  { name: "agent context (always paid)", warn: 3 * KB, fail: 6 * KB, test: (p) => /(^|\/)AGENTS\.md$/.test(p) },
  { name: "route map", warn: 4 * KB, fail: 8 * KB, test: (p) => /AGENT-MAP\.md$/.test(p) || p.startsWith(".agent/") },
  { name: "test", warn: 25 * KB, fail: 50 * KB, test: (p) => /\.test\.[jt]s$/.test(p) || /(^|\/)tests?\//.test(p) },
  { name: "source", warn: 25 * KB, fail: 50 * KB, test: (p) => /\.(rs|ts|tsx|js|mjs)$/.test(p) },
  { name: "stylesheet", warn: 25 * KB, fail: 60 * KB, test: (p) => /\.css$/.test(p) },
  { name: "template", warn: 20 * KB, fail: 50 * KB, test: (p) => /\.html$/.test(p) },
  { name: "doc", warn: 30 * KB, fail: 80 * KB, test: (p) => /^docs\/.*\.md$/.test(p) || /^README\.md$/.test(p) },
];

const SKIP = /^(package-lock\.json|.*\.(png|jpe?g|gif|webp|svg|ico|wasm|lock)$)/i;
...
for (const path of files) {
  ...
  rows.push({ path, bytes, rule: rule.name, limit: rule.fail, status, waived, reason: allow[path] ?? null });
}
...
const alwaysPaid = rows
  .filter((r) => /(^|\/)AGENTS\.md$/.test(r.path) || /AGENT-MAP\.md$/.test(r.path))
  .reduce((a, r) => a + r.bytes, 0);

if (process.argv.includes("--json")) {
  console.log(JSON.stringify({ alwaysPaid, failures, warnings, waived }, null, 2));
...
console.log(`always-paid agent context: ${kb(alwaysPaid)} (${tok(alwaysPaid)})`);
console.log(`largest file in budget scope: ${rows[0] ? `${rows[0].path} ${kb(rows[0].bytes)}` : "n/a"}`);
console.log();
...
if (waived.length) {
  console.log(`waived debt (${waived.length}) — remove from .agent-budget.json when fixed`);
  for (const r of waived) console.log(`  ${r.path}  ${kb(r.bytes)}  — ${r.reason}`);
  console.log();
}
if (warnings.length && !process.argv.includes("--check")) {
  console.log(`approaching limit (${warnings.length})`);
  for (const r of warnings.slice(0, 15)) console.log(`  ${r.path}  ${kb(r.bytes)}`);
  console.log();
}
